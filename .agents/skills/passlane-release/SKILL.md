...
## 6. Update the README "Latest release" section

`README.md` has a `## Latest release` section just below the intro (before `## Features`) that
highlights what's in the newest version. Replace its contents with the new version: a
`### [X.Y.Z](https://github.com/anssip/passlane/releases/tag/vX.Y.Z)` heading followed by the same
bullets as the changelog section from step 4. Keep the closing
`See the [full changelog](CHANGELOG.md) for earlier releases.` line.
...
## 9. Push (publishes the release — confirm first)
