...
`Entry::new("passlane_master_pwd", <vault-name>)`. Migration copies the two legacy entries to `default`/`totp` and deletes the originals, best-effort: a locked machine simply re-prompts once at the next unlock. `rename` validates all preconditions first (`vault_registry::check_rename`: new-name validity, source existence, target uniqueness), renames the registry entry, and only then moves the keychain entry — a keychain failure leaves the password filed under the old name, which merely re-prompts once at the next unlock, rather than orphaning it under a name no vault has. `remove` deletes it (a stored password for an unregistered vault would linger forever).
